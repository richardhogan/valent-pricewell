...
<r:script>
    ...
    jQuery(document).ready(function(){
  		...
  		jQuery('#wizard').smartWizard({contentURL:'${baseurl}/service/showStage?source=conceptapproved&id=' + <%=serviceProfileInstance?.id%> ,contentCache:false,  onLeaveStep:leaveAStepCallback, onShowStep: showStepCallback, selected:  <%=serviceProfileInstance?.currentStep - 1%>, labelFinish:'Assign Designer', onFinish:onFinishCallback });
...
<div id="wizard" class="swMain">
  			<ul>
				<g:each var="step" status="i" in="${ServiceStageFlow.stageSteps['conceptapproved']}">
					<li><a href="#step-${i+1}">
