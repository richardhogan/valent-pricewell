...
<r:script>
    ...
    jQuery(document).ready(function(){
    	jQuery.getScript("${baseurl}/js/jquery.smartWizard-2.0.min.js", function() {
  			...
  			jQuery('#wizard').smartWizard({contentURL:'${baseurl}/lead/showStage?source=dead&id=' + <%=leadInstance?.id%> ,contentCache:false, transitionEffect:'slide', onLeaveStep:leaveAStepCallback, onShowStep: showStepCallback, selected:  <%=leadInstance?.currentStep - 1%>});
  		});
...
<div id="wizard" class="swMain">
  			<ul>
				<g:each var="step" status="i" in="${ServiceStageFlow.leadStageSteps['dead']}">
					<li><a href="#step-${i+1}">
