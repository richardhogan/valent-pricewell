...
<r:script>
    ...
    jQuery(document).ready(function(){
    	jQuery.getScript("${baseurl}/js/jquery.smartWizard-2.0.min.js", function() {
  			...
  			jQuery('#wizard').smartWizard({contentURL:'${baseurl}/opportunity/showStage?source=closedWon&id=' + <%=opportunityInstance?.id%> ,contentCache:false, transitionEffect:'slide', onLeaveStep:leaveAStepCallback, onShowStep: showStepCallback, selected:  <%=opportunityInstance?.currentStep - 1%>});
  		});
...
<div id="wizard" class="swMain">
  			<ul>
				<g:each var="step" status="i" in="${ServiceStageFlow.opportunityStageSteps['closedWon']}">
					<li><a href="#step-${i+1}">
