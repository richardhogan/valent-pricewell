...
(function($){
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                var contentWidth = 0;
                var loader,msgBox,elmActionBar,elmStepContainer,btNext,btPrevious,btFinish, btApprove, btReject;
                ...
                function init(){
                  ...
                  if(options.approveEnabled){
                	  btApprove = $('<a>'+options.labelApprove+'</a>').attr("href","#").addClass("buttonFinish");
                	  btReject = $('<a>'+options.labelReject+'</a>').attr("href","#").addClass("buttonNext");
                	  elmActionBar.append(btReject).append(btApprove).append(btNext).append(btPrevious);
                	  
                	  $(btApprove).click(function() {
                          if(!$(this).hasClass('buttonDisabled')){
                             if($.isFunction(options.onApprove)) {
                                if(!options.onApprove.call(this,$(steps))){
                                  return false;
                                }
                             }else{
                	  ...
                	  $(btReject).click(function() {
                          if(!$(this).hasClass('buttonDisabled')){
                             if($.isFunction(options.onReject)) {
                                if(!options.onReject.call(this,$(steps))){
                                  return false;
                                }
                             }else{
                               var frm = obj.parents('form');
                ...
                function adjustButton(){
                  if(!options.cycleSteps){                
                    ...
                    if((steps.length-1) <= curStepIdx){
                      ...
                      $(btNext).addClass("buttonHide");
                    }else{
                      $(btNext).removeClass("buttonDisabled");
                      $(btNext).removeClass("buttonHide");
                    }
                  }
                  // Finish Button
                  if(options.approveEnabled){
                	  if(!steps.hasClass('disabled') || options.enableFinishButton){
                          $(btApprove).removeClass("buttonDisabled");
                          $(btReject).removeClass("buttonDisabled");
                        }else{
                        	$(btApprove).addClass("buttonDisabled");
                            $(btReject).addClass("buttonDisabled");
                        }
                  } else {
                	  if(!steps.hasClass('disabled') || options.enableFinishButton){
