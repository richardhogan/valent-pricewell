...
class RoleController {
  ...
  def show = {
	...
	{
		List membersList = UserBase.findAll("FROM UserBase ub")
    	[role: role]
	}
  }
  ...
  def save = {
    ...
    def createdRole = roleService.createRole(name, description, false, code)
		...
		if (role.protect) {
	      ...
	      response.status = 500
	    	...
	    	if (!user) {
		    	...
		    	checkUserBeforeDeleteMember(user, role)//deleteMember(user, role)
			}
		}
    }
  }
  ...
  def searchnewmembers = {
	...
	else 
	{
		...
		List userList = roleService.searchMembersNotInRole(role)
		List salesPresidentList = roleService.findUsersByRole("SALES PRESIDENT")
		List salesManagerList = roleService.findUsersByRole("SALES MANAGER")
	    log.info("Search for new role user members complete, returning $nonMembers.size records")
	    render(template: '/templates/admin/members_search', contextPath: pluginContextPath, model: [parent: role, users: userList, salesPresidentList: salesPresidentList, salesManagerList: salesManagerList])
	}
  }
