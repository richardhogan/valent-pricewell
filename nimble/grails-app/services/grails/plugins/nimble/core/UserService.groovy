...
class UserService {
	...
	public List filterUserList(List tmpList)//filter for enable and disable, also remove testUser, super admin and nobody user
	{
		List userList = new ArrayList()
		for(UserBase user : tmpList)
		{
			if(user?.enabled == true)
			{
				if(user?.username != "superadmin" && user?.username != "user" && user?.username != "nobody")
				{
					userList.add(user)
				}
			}
	...
	{
		List roleList = Role.findAll("FROM Role role")
		println roleList
    ...
    def createUser(UserBase user) {
        ...
        if (!user.hasErrors()) {
            ...
            Role defaultRole = Role.findByName(UserService.USER_ROLE)//getRoleByName("USER")
		
			
            if(!defaultRole) {
                log.error("Unable to locate default user role, aborting user creation")
				errorMsg = "defalutRoleLocate"
				isError = true
				...
				if (savedUser) {
					...
					else
					{
						...
						if (savedUser.hasErrors()) {
							...
							errorMsg = "defalutPermissionsAssign"
							isError = true
    ...
    def createLoginRecord(HttpServletRequest request) {
		try
		{
        def user = UserBase.lock(SecurityUtils.getSubject()?.getPrincipal())
        if(!user)
        {
    ...
    public def validatePass(UserBase user) {
		return validatePass(user, false)["isValidPassword"]
	}
	
    public Map validatePass(UserBase user, boolean checkOnly) {
        ...
        if (user.pass == null || user.pass.length() < grailsApplication.config.nimble.passwords.minlength) {
			...
			resultMap.put("error_message", "sort_password")
			resultMap.put("isValidPassword", false)
            return resultMap
        }
        ...
        if (user.passConfirm == null || user.passConfirm.length() < grailsApplication.config.nimble.passwords.minlength) {
			...
			resultMap.put("error_message", "sort_confirm_password")
			resultMap.put("isValidPassword", false)
            return resultMap
        }
        ...
        if (!user.pass.equals(user.passConfirm)) {
			...
			resultMap.put("error_message", "password_not_match")
			resultMap.put("isValidPassword", false)
			return resultMap
        }
        ...
        if (grailsApplication.config.nimble.passwords.mustcontain.lowercase && !(user.pass =~ /^.*[a-z].*$/)) {
			...
			resultMap.put("error_message", "not_contain_lowercase_letters")
			resultMap.put("isValidPassword", false)
			return resultMap
        }
        ...
        if (grailsApplication.config.nimble.passwords.mustcontain.uppercase && !(user.pass =~ /^.*[A-Z].*$/)) {
			...
			resultMap.put("error_message", "not_contain_uppercase_letters")
			resultMap.put("isValidPassword", false)
			return resultMap
        }
        ...
        if (grailsApplication.config.nimble.passwords.mustcontain.numbers && !(user.pass =~ /^.*[0-9].*$/)) {
			...
			resultMap.put("error_message", "not_contain_numbers")
			resultMap.put("isValidPassword", false)
            return resultMap
        }
        ...
        if (grailsApplication.config.nimble.passwords.mustcontain.symbols && !(user.pass =~ /^.*\W.*$/)) {
			...
			resultMap.put("error_message", "not_contain_symbols")
			resultMap.put("isValidPassword", false)
			return resultMap
        }
        ...
        resultMap.put("isValidPassword", true)
        return resultMap
    }
}
